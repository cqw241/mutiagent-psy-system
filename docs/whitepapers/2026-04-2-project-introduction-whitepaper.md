...
代码基线：`main` 分支，已纳入视频通话摄像头修复、`qwen3-tts-instruct-flash` 流式 TTS、DashScope 代理绕过与语音自动播报改造
...
截至 2026 年 4 月 24 日，项目已完成从线性单链路原型向 LangGraph 多智能体并行架构的升级，后端已落地 9 节点图拓扑、文本与语音并行分析、条件转介、双层 RAG 检索增强、单机会话恢复、REST 与 WebSocket 双接口，以及 React 前端的视频通话模式、Edge 面部分析与基于阿里云百炼 `qwen3-tts-instruct-flash` 的流式 TTS 回放。视频通话中，文本和语音触发的模型回复都会自动请求语音播报；普通语音输入回合仍会自动请求语音回复。当前版本已完成 RAGFlow 本地 BGE-M3 embedding 联调，并在前端 Trace 面板展示 RAG 命中状态。当前仓库测试目录中已包含 130 个后端 pytest 测试函数，另有 26 个前端 `node:test` 纯逻辑用例，表明该系统已具备“可联调、可演示、可持续硬化”的试点前工程基础。
...
- `qwen3-tts-instruct-flash` / Qwen TTS 非流式 URL 下载、SSE 流式分片、重试、回退与 DashScope 代理绕过分支
...
| 视频通话 / TTS | 支持视频通话模式、语音输入回合自动播报、视频通话内文本/语音回复自动播报，以及基于 `qwen3-tts-instruct-flash` 的流式语音回放 |
