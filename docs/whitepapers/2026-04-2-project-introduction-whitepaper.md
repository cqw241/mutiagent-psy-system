...
代码基线：`main` 分支，已纳入视频通话摄像头修复、`qwen-tts-latest` 流式 TTS、DashScope 代理绕过与语音自动播报改造；2026-05-22 已补充下一阶段研发决策，当前以 `docs/plans/2026-05-22-project-diagnosis-and-development-plan.md` 为后续计划基线。
...
截至 2026 年 5 月 22 日，项目已完成从线性单链路原型向 LangGraph 多智能体并行架构的升级，后端已落地 9 节点图拓扑、文本与语音并行分析、条件转介、双层 RAG 检索增强、单机会话恢复、REST 与 WebSocket 双接口，以及 React 前端的视频通话模式、Edge 面部分析与基于阿里云百炼 `qwen-tts-latest` 的流式 TTS 回放。视频通话中，文本和语音触发的模型回复都会自动请求语音播报；普通语音输入回合仍会自动请求语音回复。当前版本已完成 RAGFlow 本地 BGE-M3 embedding 联调，并在前端 Trace 面板展示 RAG 命中状态。当前仓库测试目录中已包含 130 个后端 pytest 测试函数，另有 26 个前端 `node:test` 纯逻辑用例，表明该系统已具备“可联调、可演示、可持续硬化”的试点前工程基础。

2026 年 5 月 22 日后续研发决策已经收敛：早期 `docs/plans` 分阶段计划文件已作为冗余历史清理，当前以 `docs/plans/2026-05-22-project-diagnosis-and-development-plan.md` 为唯一后续计划入口；试点前产品形态先按“学生端 + 单一管理员”推进；系统允许保存完整对话文本用于审计和复盘，但默认不保存原始视频，原始音频默认不长期保存；本地大模型接入被放入 3-6 个月能力扩展阶段，不作为短期试点硬化阻塞项。
...
下一阶段建议将 WebSocket 契约升级为稳定事件协议：`final` 事件继续保留现有字段，同时补充顶层 `risk_level`、`alert_status`、`alert_event_id`；高风险时额外发送 `risk_event`，只包含事件编号、风险级别、处置状态、脱敏摘要和 trace id。外部系统不应依赖 `trace` 的内部调试结构作为集成协议。
...
`face_analyzer` 当前已完成**端云协同计算（Edge-Cloud Collaboration）**升级，不再仅仅是占位路由。系统实现了：

- 基于 `@mediapipe/tasks-vision` FaceLandmarker 的纯前端本地边缘推断，严守“原始视频级数据绝不上传云端”的安全契约。
- 基于 30+ 关键面部点自动提取，并在本地转换为后端能够客观认识的 FACS (Action Units) 标准面部块序列组。
- 为了应对视频微积分的瞬态噪声，在本地实现基于 1.25 秒固定滑动窗口的防抖缓冲池化（Average Pooling）。
- 后端基于多路 FACS 时序集合，配合人工启发规则池输出明确观察事实（如：用户连续处于紧皱眉头）。
- 在接入主链路后的 LLM System Prompt 环节追加强硬系统级防护：“面部观察结果仅有资格作为轻度条件上下文辅助评价，无法在核心逻辑跳变上扮演唯一证据。”

因此，项目当前已经具备完整的“文本 + 语音可用 + 端侧实时本地面部辅助”的多模态底座。
...
- 当前联调环境支持通过本地 BGE-M3 embedding 服务接入 RAGFlow 的 LocalAI embedding 配置，从而降低云端 embedding key、配额或网络波动对检索链路的影响
- Dify-compatible retrieval 请求会显式传入 `metadata_condition: null`，避免 RAGFlow 将空 metadata 条件解释为过滤全部文档
...
前端已内置 Trace 面板，用于展示音频片段、支持等级、风险校准、emotion2vec 当前状态、声学线索，以及“专业案例库 / 同辈支持库”的 RAG 检索状态。这使系统在联调和试点阶段具备一定程度的过程透明度，有助于减少“黑箱感”。
...
- 普通语音输入回合自动请求语音回复；视频通话模式中，文本和语音回合都会自动请求语音回复
- 本地摄像头接入与端侧面部分析开关，预览 `<video>` 节点在通话面板中常驻，打开镜头时可稳定绑定摄像头流，原始画面不上传
- 阶段状态提示，如接收、检索、评估、生成回复等
- 最新转写结果展示
- Trace 调试侧栏，包含 RAG 检索状态卡片
- 高风险支持卡片渲染
- 基于 `tts_audio` / `tts_end` 事件的流式语音回复排队播放，前端可将 Qwen 流式 PCM 片段即时封装为 WAV 后播放
- 后端 DashScope LLM/TTS 请求会绕过本机代理环境变量，降低 `HTTPS_PROXY` 等本地代理导致的 TLS 连接中断风险；TTS 在 DashScope 未产出音频时保留 `edge_tts` 降级路径
...
- `qwen-tts-latest` / Qwen TTS 非流式 URL 下载、SSE 流式分片、重试、回退与 DashScope 代理绕过分支
- `peer_support_retriever` 的启用、关闭与异常降级分支
- WebSocket `final.trace` 中 emotion2vec 状态暴露
- 语音 transcript 片段分发与空消息保护
- 前端流式 assistant 收尾、语音 transcript 去重、视频通话自动语音回复、摄像头预览挂载，以及流式 PCM 音频的浏览器播放适配
- 配置与 checkpoint 工厂
...
- LLM 不可用
- RAGFlow 不可用
- 云端 embedding key、配额或网络不可用时，可切换到本地 BGE-M3 embedding 服务
- Whisper 模型不可加载
...
- Redis 会话态协同
- 更完整的会话审计和回放机制
- 告警事件表与会话审计表，至少覆盖事件编号、触发时间、风险证据、送达状态、处理状态、管理员操作时间线和回执
...
在真实校内 SOP 尚未确定前，系统默认采用最小可执行 SOP：`created -> delivered -> acknowledged -> in_progress -> escalated -> closed`。单一管理员先承担告警查看、接单、升级、回执和结案职责，后续再拆分辅导员、心理中心和系统管理员等角色。
...
风险评测建议采用“两层标签”方案：工程侧先把现有 34 个风险种子案例整理为 golden case baseline，自动评估高危召回、非高危误报、边界表达和 RAG 命中；进入试点前再由心理中心或伦理评审对高风险、误报、漏报样本进行抽检确认。工程 baseline 可以驱动日常回归，但不能替代专业审核。
...
第一版产品不建议一次性建设复杂多角色后台，应先落地“学生端 + 单一管理员”：学生端保留文本、语音、视频通话与高风险支持卡片；管理员端只做告警列表、事件详情、接单、升级、回执、结案和基础指标。

### 11.5 数据保存与隐私边界

当前后续决策允许保存完整对话文本，用于高风险审计、误报漏报复盘和模型/规则评测。建议默认策略如下：

- 完整对话文本与 ASR 转写：保留 180 天，用于审计、复盘和质量改进。
- 高风险事件摘要、处置状态、管理员回执和时间线：保留 3 年，用于校内安全审计和责任追踪。
- 原始视频：不上传、不保存，只在端侧提取结构化 FACS/AU 特征。
- 原始音频：默认不长期保存；如试点研究确需保存，必须单独开启配置、明确授权、限定访问权限和保留期限。
- 声学特征、emotion2vec 状态、面部结构化观察和 trace 摘要：按最小必要原则保存，避免将辅助信号包装成诊断结论。
...
- 接入生产级 checkpoint 后端
- 完善告警落库与审计字段
- 按“学生端 + 单一管理员”落地最小高风险人工处置 SOP
- 建立风险评测 harness v0，并将现有风险种子案例转为 golden case baseline
- 稳定 WebSocket `final` / `risk_event` 对外事件契约
- 增加 CI、测试报告和部署脚本
...
- 评估误报率、转介触达率和平均响应时延
- 建立人工复核台账
- 根据校内真实 SOP 将单一管理员职责拆分为辅导员、心理中心、系统管理员等正式角色

### 阶段三：能力扩展

- 接入本地大模型推理服务，并通过风险评测 baseline 与云端模型做回归对比
- 引入更完整的视频或时序情绪建模
- 构建面向管理端的风险运营看板
...
与此同时，项目也清楚地保留了边界感：不做诊断、不假装生产就绪、不把规划中的能力写成已上线能力。这种克制对高校心理支持场景尤为重要。基于当前仓库状态，本项目适合作为学校心理支持数字化建设中的“智能辅助底座”继续推进。
...
| RAG | 支持风险案例检索与同辈话术检索，支持本地 BGE-M3 embedding 联调，两条链路均可降级 |
| 语音增强 | 支持可选 `emotion2vec_plus_large` |
| 面部能力 | 纯本地端侧 MediaPipe FACS 提取，后端基于降噪滑动窗口进行特征关联判断 |
| 视频通话 / TTS | 支持视频通话模式、语音输入回合自动播报、视频通话内文本/语音回复自动播报，以及基于 `qwen-tts-latest` 的流式语音回放 |
| 高风险闭环 | 强制进入转介与 webhook 告警 |
| 自动化测试 | 当前仓库测试目录中已包含 130 个后端 pytest 测试函数，另有 26 个前端 `node:test` 用例 |
